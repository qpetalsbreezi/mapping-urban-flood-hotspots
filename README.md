...
## Project Overview
This project maps recurring urban flood hotspots in **Raleigh, NC** and **Houston, TX** using Sentinel‑1 SAR imagery combined with NOAA Storm Events flood reports. The study period is **2015–2025**, aligned with Sentinel‑1 availability. For each city, flood events are filtered from NOAA using location and county criteria (USGS gage height is recorded in the event catalog but **not** used as an inclusion filter), matched to pre‑ and post‑storm Sentinel‑1 image pairs, converted into per‑event flood masks, and aggregated into multi‑event flood frequency maps. Validation uses independent NOAA locations from events without SAR coverage against an ever‑flooded map built from SAR events.
...
- `code/event_selection/filter_noaa_events.py` – Filters NOAA Storm Events by state, county, event type, and keywords (no USGS filter).
...
- `docs/event_selection_methodology.md` – Event selection (NOAA with coordinates, 2015–2025; USGS metadata only).
...
- `docs/paper_reviewer_comment_changes.md` – Reviewer-driven methodology revisions, final validation numbers, and event catalog (#18).
...
1. **Event Selection (2015–2025)**  
   Filter NOAA Storm Events for Wake County (Raleigh) and Harris County (Houston) using location keywords; include events with coordinates in the Sentinel‑1 era. USGS gage readings (Crabtree Creek, Buffalo Bayou) are attached in the event catalog for context but do not gate inclusion. Events before 2015 are excluded because Sentinel‑1 was not operational.
